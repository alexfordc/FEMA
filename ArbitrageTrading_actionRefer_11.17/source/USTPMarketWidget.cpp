									   ...
									   const QString& ssPrice, const QString& bbQty, const QString& bsQty, const QString& sbQty, const QString& ssQty)
{
	...
	if (itemList.size() == 0){
		...
		QString vol = QString::number(USTPMutexId::getActionNum(firstIns)); 	 
		addItem(nRow, 3, vol);
...
void USTPMarketWidget::doUpdateDepthMd(const QString& complexIns, const QString& reqKey, const int& rowId, const int& direction, const double& firstPrice, const double& secPrice, const double& thirdPrice)
{
	emit onUpdateComplexInsPrice(reqKey, rowId, direction, firstPrice, secPrice, thirdPrice);
}
