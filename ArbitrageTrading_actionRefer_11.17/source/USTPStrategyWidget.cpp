...
{	
	...
	connect(pMarketWidget, SIGNAL(onUpdateComplexInsPrice(const QString&,  const int&, const int&, const double&, const double&, const double&)), this, SLOT(doUpdateComplexInsPrice(const QString&, 
										   ...
										   bool isDefineOrder, bool isOpponentPriceType, bool isCancelFirstOrder, bool isCancelSecOrder, bool isCylce, bool isActionReferTick)
{
	...
	firstTimeSpinBox->setRange(1, 60000); 
	...
	secondTimeSpinBox->setRange(1, 60000); 
...
void USTPStrategyWidget::createNewOrder(const int& row)
{
	...
	char direction = (bsItems.at(0) == QString(tr("0"))) ? USTP_FTDC_D_Buy : USTP_FTDC_D_Sell;

	QComboBox* offsetCombox = qobject_cast<QComboBox *>(cellWidget(row, 4));
	QStringList offsetItems = offsetCombox->currentText().split("-");
	char offstFlag = (offsetItems.at(0) == QString(tr("0"))) ? USTP_FTDC_OF_Open : USTP_FTDC_OF_Close;
	...
	char hedgeFlag = (hedgeItems.at(0) == QString(tr("0"))) ? USTP_FTDC_CHF_Speculation : USTP_FTDC_CHF_Arbitrage;
	...
	createStrategy(orderType, firstName, secName, price, oneOpenSlip, secOpenSlip, qty, direction, offstFlag, hedgeFlag, 
		...
		isOppnentPrice, isDefineOrder, isActionRefer);
}
...
void USTPStrategyWidget::createStrategy(const int& orderType, const QString& firstIns, const QString& secIns, const double& orderPriceTick, const int& firstPriceSlipPoint, const int& secPriceSlipPoint, 
										...
										const int& actionSuperNum, bool isAutoFirstCancel, bool isAutoSecCancel, bool isCycle, bool isOppentPrice, bool isDefineOrder, bool isActionRefer)
{
	switch (orderType){
		case 0:
			{
				...
				USTPStrategyBase* pStrategy = new USTPStarePriceArbitrage(label, firstIns, secIns, orderPriceTick, secPriceSlipPoint, qty, bs, 
					offset, hedge, cancelFirstTime, cancelSecTime, cycleStall, isAutoFirstCancel, isAutoSecCancel, isCycle, mOrderWidget, mCancelWidget, this);
		...
		case 1:
			{
				...
				USTPStrategyBase* pStrategy = new USTPConditionArbitrage(label, firstIns, secIns, orderPriceTick, firstPriceSlipPoint, secPriceSlipPoint, qty, bs, 
					...
					isDefineOrder, isActionRefer, mOrderWidget, mCancelWidget, this);
				mUserStrategys.insert(label, pStrategy);
			}
			break;
		case 2:
		...
		case 3:
			{
				...
				USTPStrategyBase* pStrategy = new USTPConditionStareArbitrage(label, firstIns, secIns, orderPriceTick, firstPriceSlipPoint, secPriceSlipPoint, qty, bs, 
					offset, hedge, cancelFirstTime, cancelSecTime, cycleStall, isAutoFirstCancel, isAutoSecCancel, isCycle, isOppentPrice, isDefineOrder, mOrderWidget, mCancelWidget, this);
				mUserStrategys.insert(label, pStrategy);
			}
			break;
		case 4:
										 ...
										 const int& actionSuperNum, bool isAutoFirstCancel, bool isAutoSecCancel,bool isCycle, bool isOppentPrice, bool isDefineOrder, bool isAllTraded, bool isReferTick, const int& orderType)
{
	if(mUserStrategys.find(orderLabel) != mUserStrategys.end()){
		...
		if(isAllTraded && isCycle){
			if((USTP_FTDC_D_Buy == bs) && (USTP_FTDC_OF_Open == offset)){
				double nextOrderPrice = orderPriceTick + cycleStall * USTPMutexId::getInsPriceTick(firstIns);
				createStrategy(orderType, firstIns, secIns, nextOrderPrice, firstPriceSlipPoint, secPriceSlipPoint, qty, USTP_FTDC_D_Sell, USTP_FTDC_OF_Close, hedge, 
					cancelFirstTime, cancelSecTime, cycleStall, actionReferNum, actionSuperNum, isAutoFirstCancel, isAutoSecCancel, isCycle, isOppentPrice, isDefineOrder, isReferTick);
			}else if ((USTP_FTDC_D_Sell == bs) && (USTP_FTDC_OF_Open == offset)){
				double nextOrderPrice = orderPriceTick - cycleStall * USTPMutexId::getInsPriceTick(firstIns);
				createStrategy(orderType, firstIns, secIns, nextOrderPrice, firstPriceSlipPoint, secPriceSlipPoint, qty, USTP_FTDC_D_Buy, USTP_FTDC_OF_Close, hedge, 
					cancelFirstTime, cancelSecTime, cycleStall, actionReferNum, actionSuperNum, isAutoFirstCancel, isAutoSecCancel, isCycle, isOppentPrice, isDefineOrder, isReferTick);
			}else if ((USTP_FTDC_D_Buy == bs) && (USTP_FTDC_OF_Close == offset)){
				double nextOrderPrice = orderPriceTick + cycleStall * USTPMutexId::getInsPriceTick(firstIns);
				createStrategy(orderType, firstIns, secIns, nextOrderPrice, firstPriceSlipPoint, secPriceSlipPoint, qty, USTP_FTDC_D_Sell, USTP_FTDC_OF_Open, hedge, 
					cancelFirstTime, cancelSecTime, cycleStall, actionReferNum, actionSuperNum, isAutoFirstCancel, isAutoSecCancel, isCycle, isOppentPrice, isDefineOrder, isReferTick);
			}else if ((USTP_FTDC_D_Sell == bs) && (USTP_FTDC_OF_Close == offset)){
				double nextOrderPrice = orderPriceTick - cycleStall * USTPMutexId::getInsPriceTick(firstIns);
				createStrategy(orderType, firstIns, secIns, nextOrderPrice, firstPriceSlipPoint, secPriceSlipPoint, qty, USTP_FTDC_D_Buy, USTP_FTDC_OF_Open, hedge, 
					cancelFirstTime, cancelSecTime, cycleStall, actionReferNum, actionSuperNum, isAutoFirstCancel, isAutoSecCancel, isCycle, isOppentPrice, isDefineOrder, isReferTick);
			}
