...
{	
	...
	mHedgeCheckBox = new QCheckBox(tr("套利"));
	mHedgeCheckBox->setChecked(false);
	...
	mGridLayout->addWidget(mHedgeCheckBox, 0, 0, 1, 1);	
...
{	
	...
	char direction = (bsItems.at(0) == tr("0")) ? USTP_FTDC_D_Buy : USTP_FTDC_D_Sell;
	...
	{
	case 0:
		offstFlag = USTP_FTDC_OF_Open;
		break;
	case 1:
		offstFlag = USTP_FTDC_OF_Close;
		break;
	case 2:
		offstFlag = USTP_FTDC_OF_CloseToday;
		break;
	case 3:
		offstFlag = USTP_FTDC_OF_CloseYesterday;
		break;
	default:
		offstFlag = USTP_FTDC_OF_Open;
		break;
	}
	bool isHedge = mHedgeCheckBox->isChecked();
	char hedgeFlag = isHedge ? USTP_FTDC_CHF_Arbitrage : USTP_FTDC_CHF_Speculation;
	int qty = mVolumeSpinBox->value();
	double price = mPriceSpinBox->value();
	QString speLabel;
	if(USTP_FTDC_D_Buy == direction)
		speLabel = insId + tr("_B");
	else
		speLabel = insId + tr("_S");
	emit onSubmitOrder(label, speLabel, insId, direction, hedgeFlag, offstFlag, qty, price);
}
	...
	foreach(QString ins, inss){
		...
		if (items.size() == 2){
			...
			if(iter == mInss.end()){
				...
				USTPStrategyBase* pStrategy_B = new USTPSpeculateOrder("", label_B, "", "", 0.0, 0, USTP_FTDC_D_Buy,  USTP_FTDC_OF_Open,
					USTP_FTDC_CHF_Speculation, 0, 0, 0, 0, 0, false, false, false, mOrderWidget, mCancelWidget, this);
				USTPStrategyBase* pStrategy_S = new USTPSpeculateOrder("", label_S, "", "", 0.0, 0, USTP_FTDC_D_Sell,  USTP_FTDC_OF_Open,
					USTP_FTDC_CHF_Speculation, 0, 0, 0, 0, 0, false, false, false, mOrderWidget, mCancelWidget, this);
