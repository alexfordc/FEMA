...
{	
	...
	mHeaders << CANCEL_WIDGET_TAB_HEAD_0 << CANCEL_WIDGET_TAB_HEAD_1 << CANCEL_WIDGET_TAB_HEAD_2
		...
		<< CANCEL_WIDGET_TAB_HEAD_6 << CANCEL_WIDGET_TAB_HEAD_7;
...
{	
	...
	if (itemList.size() == 0){
		if(status == USTP_FTDC_OS_AllTraded || status == USTP_FTDC_OS_Canceled)
			return;
		...
		addItem(nRow, 5, getOffsetFlagText(offsetFlag));
		addItem(nRow, 6, QString::number(orderPrice));
		setCellWidget(nRow, 7, pCancelBtn);
