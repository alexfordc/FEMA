...
{	
	connect(USTPCtpLoader::getMdSpi(), SIGNAL(onUSTPRtnDepthMarketData(const QString&, const double&, const double&, const double&, const double&, const int&, const double&, 
		const double&, const double&, const double&)), this, SLOT(doUSTPRtnDepthMarketData(const QString&, const double&, const double&, const double&, 
		const double&, const int&, const double&, const double&, const double&, const double&)), Qt::QueuedConnection);
	...
	connect(this, SIGNAL(onUpdateDepthMd(const QString&, const QString&, const int&, const int&, const double&, const double&, const double&)), pMarketWidget, SLOT(doUpdateDepthMd(const QString&, 
		const QString&, const int&, const int&, const double&, const double&, const double&)), Qt::QueuedConnection);
}
...
void USTPComplexMd::doUSTPRtnDepthMarketData(const QString& instrumentId, const double& preSettlementPrice, const double& openPrice, const double& lastPrice,
											 const double& bidPrice, const int& bidVolume, const double& askPrice, const double& askVolume, const double& highestPrice, 
...
void USTPComplexMd::doGetDepthMd(const QString& complexIns, const QString& reqKey, const int& direction, const int& rowIndex)
{
	if(mComplexKey == complexIns){
		...
		{
		case 0:
			emit onUpdateDepthMd(mComplexKey, reqKey, rowIndex, direction, mFirstBidPriceDiff, mSecondBidPriceDiff, mThirdBidPriceDiff);
			break;
		case 1:
			emit onUpdateDepthMd(mComplexKey, reqKey, rowIndex, direction, mFirstAskPriceDiff, mSecondAskPriceDiff, mThirdAskPriceDiff);
			break;
		}
	}
}
