...
{	
	...
	if(!mThis->m_pBase64->getDateIsValid()){
		return false;
	}
...
{
	mThis->m_pMdApi = CUstpFtdcMduserApi::CreateFtdcMduserApi("../log/");
	mThis->m_pMdApi->RegisterFront(mThis->mMdPath.toLatin1().data());
	mThis->m_pMdApi->SubscribeMarketDataTopic(100, USTP_TERT_QUICK);
...
{
	mThis->m_pTradeApi = CUstpFtdcTraderApi::CreateFtdcTraderApi("../log/");	
	mThis->m_pTradeApi->SubscribePublicTopic(USTP_TERT_QUICK);
	mThis->m_pTradeApi->SubscribePrivateTopic(USTP_TERT_QUICK);
...
CUstpFtdcMduserApi* USTPCtpLoader::getMdApi()
{
	return mThis->m_pMdApi;
}

CUstpFtdcTraderApi* USTPCtpLoader::getTradeApi()
{
